<?php
// ======================================================================
// GOPAY FREE FIRE - WAR EDITION (Fixed Lead + Staggered Inquiry)
//
// Strategi:
...
const INQUIRY_STAGGER_MS     = 10;           // 10 slot mencakup 252ms; tidak memakai akhir_lead.txt.
...
function runStaggeredInquiry(
    ...
    $successMap = [];      // userId -> ['order','orderId','headers']
...
function gpyPay(): void {
    ...
    $lines = file('user_server_ff.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [];
    $orders = [];
    $seenOrders = [];
    $duplicateOrders = [];
    $invalidOrderLines = [];
    ...
    if (!empty($invalidOrderLines)) {
        throw new RuntimeException(
            "user_server_ff.txt memuat " . count($invalidOrderLines)
            . " baris tidak valid; setiap baris wajib memuat userId"
        );
    }
    if (!empty($duplicateOrders)) {
        throw new RuntimeException(
            "user_server_ff.txt memuat user duplikat: "
